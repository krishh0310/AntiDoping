...
def module_detail(request, module_id):
    if not _verify_module_access(request, module_id):
        return render(request, 'error.html', {'error': 'Access denied. Complete previous modules first.'}, status=403)
...
def module_part(request, module_id, part_order):
    ...
    return render(request, 'module_part.html', {
        'module':         module,
        'part':           current_part,
        'parts':          parts,
        'prev_part':      prev_part,
...
def module_quiz(request, module_id):
    ...
    if request.method == 'POST':
        ...
        return render(request, 'module_quiz.html', {
            'module':      module,
            'questions':   questions,
            'submitted':   True,
            'score':       score,
...
def final_quiz(request):
    ...
    if request.method == 'POST':
        ...
        return render(request, 'final_quiz.html', {
            'questions':  questions,
            'submitted':  True,
            'score':      score,
            'total':      total,
...
def chat_api(request):
    ...
    try:
        ...
        completion = client.chat.completions.create(
            ...
            messages=[
                {'role': 'system', 'content': 'You are an anti-doping awareness assistant. Keep answers clear and student-friendly.'},
